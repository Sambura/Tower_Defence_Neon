...
public class ChainTower : Tower 
{
	...
	{
		var damageLine = new Geometry2D.Segment(transform.position, other.transform.position);
	...
	private void Bind(ChainTower other, Geometry2D.Segment damageLine, GameObject laser, float gunRotation)
	{
		...
		if (bindings.Count > 0)
		{
			foreach (var binding in bindings)
			{
				...
				do
				{
					...
					var translation = new Geometry2D.Segment(enemy.LastPosition, enemy.transform.position);
	...
	private class Binding
	{
		...
		public Geometry2D.Segment damageLine;
...
public class Geometry2D
{
	...
	public struct Segment
	{
		...
		// Basically checks, if point belongs to this segment, if this point belongs to line containing this segment
	...
	static public Vector2 SegmentIntersection(Segment a, Segment b)
	{
		...
		if (c == Vector2.negativeInfinity) return c; // Segments are parallel but doesn't lie on the same line, no intersection

		if (c == Vector2.positiveInfinity)
			if (a.IsBelongToRect(b.a) || a.IsBelongToRect(b.b))
				return c; // Segments lie on the same line and have common points - many intersections
			else
